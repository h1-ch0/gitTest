...
taiTwo_count_df = pd.read_excel('/Volumes/YAHO_/006.일일 보관/INPUTS_update_0417/스타벅스_300_2500_all_count_tai_2.xlsx')
...
def Fig_Pearson_tai_tai(taiOne_df,taiTwo_df, count_columns):
    ...
    for column in count_columns:
        ...
        plt.scatter(column, taiTwo_pr,marker = '*', s= 240, c = 'darkgreen')
        plt.scatter(column, taiOne_pr,marker = 'o', s= 130,c = 'red')
...
def Fig_Kendalltau_tai_tai(taiOne_df,taiTwo_df, count_columns):
    ...
    # plt.tight_layout(w_pad=1)
    plt.xticks(np.arange(100, 1600, 100))
    ax.set_yticklabels(ax.get_yticks(), fontsize = 20, fontweight = 'bold')
    ax.set_xticklabels(ax.get_xticks(), fontsize = 20, fontweight = 'bold')
    ax.xaxis.set_major_locator(MultipleLocator(500))
    ax.xaxis.set_minor_locator(MultipleLocator(100))
