...
def Joint_SplinePlot_FixedCentralityBins(cent_df, count1_df,count2_df, column = 1000,set_bins = False,\
    ...
    ax_main1.set_ylabel(f'[{tai1}] 판매점 개수' , labelpad = 10 , fontsize = 36, fontweight = 1000) ## 산포도 y 라벨
    ...
    ax_main2.set_ylabel(f'[{tai2}] 지점 개수' , labelpad = 10 , fontsize = 36, fontweight = 1000) ## 산포도 y 라벨
    ax_main2.legend(loc='upper left')
    
    #저장여부
    if save == True: #저장을 할 지, 플롯을 보여줄 지 여부에 따라 실행. 기본값은 False
        plt.savefig(f'joint_{column}m_kde(bw={bandwidth:.2f})(x=0.00)_May27.png')
    else:
        plt.show()
        
        
x = np.arange(300,2510,100)
for column in x:        
